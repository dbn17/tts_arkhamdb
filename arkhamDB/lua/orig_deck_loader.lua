...
function onLoad()
  ...
  init()
  -- Setup...
  publicDeckURL="https://es.arkhamdb.com/api/public/decklist/"
  privateDeckURL="https://es.arkhamdb.com/api/public/deck/"
  cardURL="https://es.arkhamdb.com/api/public/card/"
  subnameCards={{name="El Necronomicón",xp=0},{name="Signos Arcaicos",xp=3},{name="Solución extraña",xp=4},{name="Reliquia de las eras",xp=0}, {name="Piedra antigua",xp=4}}
...
function checkboxPPClicked()
  ...
  for k,v in pairs(buttons) do
    if (v.label == "Mazo privado") then
      local button_parameters = {}
      button_parameters.label = "Mazo público"
      button_parameters.index = v.index
      tile.editButton(button_parameters)
      privateDeck = false
    else
      if (v.label == "Mazo público") then
        local button_parameters = {}
        button_parameters.label = "Mazo privado"
        button_parameters.index = v.index
        tile.editButton(button_parameters)
        privateDeck = true
      end
...
function checkboxPermsClicked()
  buttons = tile.getButtons()
  for k,v in pairs(buttons) do
    if (v.label == "Permanentes") then
      local button_parameters = {}
      button_parameters.label = "Sin permanentes"
      button_parameters.index = v.index
      tile.editButton(button_parameters)
      permanents = false
    else
      if (v.label == "Sin permanentes") then
        local button_parameters = {}
        button_parameters.label = "Permanentes"
        button_parameters.index = v.index
        tile.editButton(button_parameters)
        permanents = true
      end
...
function deckReadCallback(req)
  ...
  then
    ...
    then
      broadcastToAll("El mazo privado "..deckID.." no esta compartido", {0.5,0.5,0.5})
      return
    end
  ...
  if (JsonDeckRes == nil)
  then
    broadcastToAll("¡Mazo no encontrado!", {0.5,0.5,0.5})
    return
  else
    print("Encontrada lista de cartas: "..JsonDeckRes.name)
  end
...
function cardReadCallback(req)
  ...
  then
    cardList[JsonCardRes.code].cardName = JsonCardRes.name
  else
    cardList[JsonCardRes.code].cardName = JsonCardRes.name .. " (" .. JsonCardRes.xp .. ")"
  end

  -- Check for subname
  for k,v in pairs(subnameCards) do
    if (v.name == JsonCardRes.name and (v.xp == JsonCardRes.xp or JsonCardRes.xp == nil))
    then
  ...
  if permanents then
    if (JsonCardRes.permanent == true or extraPermanents[JsonCardRes.name]) then
  ...
  for k,v in pairs(allCards) do
    ...
    then
      ...
      then
        ...
        print('Añadida '.. cardCount .. ' x ' .. cardName)
        return
      end
    end
  end
  broadcastToAll("Carta no encontrada: "..cardName, {0.5,0.5,0.5})
end
...
function cardTaken(card, params)
  ...
  if (card.getName() == params[1]) then
    ...
    print('Carta erronea: ' .. card.getName())
...
function makeButton()
  ...
  button_parameters.tooltip = "Pulsa para crear el mazo"
...
function makeCheckboxPP()
  ...
  checkbox_parameters.tooltip = "Pulsa para cambiar entre ID de mazo Privado/Público"
  checkbox_parameters.label = "Mazo privado"
...
function makeCheckboxPerms()
  ...
  checkbox_parameters.width = 1350
  checkbox_parameters.height = 300
  checkbox_parameters.tooltip = "Pulsa para separar las cartas Permanentes"
  checkbox_parameters.label = "Permanentes"
  checkbox_parameters.font_size = 200
  checkbox_parameters.scale = {0.1,0.1,0.1}
  tile.createButton(checkbox_parameters)
end
